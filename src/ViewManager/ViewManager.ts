...
import {penmanCache, PredicateTooltipData, SingleLink, WikiTooltipData} from "../util";
...
export class ViewManager {
    ...
    private static readonly DEFAULT_TEXT_VALUE = "After seeing that YouTube video I wonder, what does the fox say?";
    ...
    private static HomepageView() {
        ...
        const textAndButtonDiv = $("<div/>")
            ...
            .appendTo(inputDiv);
        const textAndError = $("<div class='input-error-container' />")
            .appendTo(textAndButtonDiv);
        const textInput = new TextInput(
            $("<div/>")
                .appendTo(textAndError),
            InputType.SENTENCE,
            "query"
        );
        ...
        mainForm.on("submit", (evt) => {
            ...
            ViewManager.requestHandler(evt,
                requestType,
                requestEndPoint,
                textInput,
                textAndError,
    ...
    private static requestHandler(evt: SubmitEvent,
        ...
        if (textInput.value() == '')
            return;
        if (requestType === RequestType.SENTENCE && String(textInput.value()).split(/\s+/).length > 512) {
    ...
    private static SentenceToAMR(data: {
        ...
        const sentenceTextInput = new TextInput(sentenceText, InputType.SENTENCE, "sentence");
        sentenceTextInput.value(data.sentence);
        const parseButton = $("<button type='submit'>")
            .appendTo(sentenceText);
        new Button(parseButton, ButtonType.MINI, "PARSE");

        const penmanForm = $("<form class='penman-form' />")
            .appendTo(inputContainer);
        penmanForm.append(
            $("<p class='input-label'/>")
                .text("PENMAN")
        ...
        const penmanTextInput = new TextInput(penmanText, InputType.PENMAN, "penman", "80%");
        penmanText.removeAttr("style")
            .css("flex-grow", "1")
        penmanTextInput.value(data.penman);
        const buttonContainer = $("<div class='penman-buttons'/>").appendTo(penmanText);
        ...
        const update = $("<button type='button' class='penman-submit' />").appendTo(updateModifyContainer);
        const modify = $("<button type='button' class='penman-submit' />").appendTo(updateModifyContainer);
        ...
        const updateButton = new Button(update, ButtonType.MINI, "UPDATE");
        const modifyButton = new Button(modify, ButtonType.MINI, "SUGGEST AN EDIT");
        new Resizer(resizer);
        ...
        updateButton.onClick(evt => {
            evt.preventDefault();
            if (penmanTextInput.value() === currentPenman.value.penman)
                return;
            ViewManager.requestHandler(evt,
                RequestType.PENMAN,
    ...
    private static AMRToSentence(data: {
        ...
        penmanTextInput.value(data.penman);
        const buttonContainer = $("<div class='penman-buttons'/>").appendTo(penmanText);
        const generate = $("<button type='submit' class='penman-submit generate-btn' />").appendTo(buttonContainer);
        ...
        const sentenceTextInput = new TextInput(sentenceText, InputType.SENTENCE, "sentence");
        sentenceTextInput.value(data.sentence);
        const modifyButton = $("<button type='submit'>")
            .appendTo(sentenceText);
        new Button(modifyButton, ButtonType.MINI, "SUGGEST AN EDIT");
    ...
    private static errorHandler(jqXHR, container) {
        ...
        if (checkErr)
            checkErr.remove();
        const errAlert = $("<div id='alert' />")
            .appendTo(container);
        if (jqXHR.readyState === 0)
            new Alert(errAlert, AlertType.ERROR, "Error", "Could not reach the server.");
        else
            switch (jqXHR.status) {
                case 429:
                    new Alert(errAlert, AlertType.ERROR, "Error: Too many requests", "Try again later.");
                    break;

                default:
                    const response = jqXHR.responseJSON;
                    new Alert(errAlert, AlertType.ERROR, "Error: " + response.message, response.text);
                    break;
            }
    }
}